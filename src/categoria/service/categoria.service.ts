...
export class CategoriaService {
  ...
  public async create(createCategoriaDto: CreateCategoriaDto) {
    ...
    if (!loja) {
      throw new NotFoundException(
        `Loja com ID ${createCategoriaDto.idLoja} não encontrada`,
      );
    }
  ...
  public async findAll(lojaId: number): Promise<Categoria[]> {
    ...
    const categorias = await this.categoriaRepository
      ...
      .catch(() => {
        throw new NotFoundException(
          'Nenhuma categoria encontrada para esse id de loja.',
        );
      });

      const categoriasSemSenha = categorias.map((categoria) => {
        delete categoria.loja;
        return categoria;
      });

    return categoriasSemSenha;
    ...
    return await this.categoriaRepository
      ...
      .catch((error) => {
        ...
        throw new BadRequestException(
          'Não foi possível encontrar as categorias',
        );
      });
  }
  ...
  public async updateProdutos(id: number, produtos: string[]) {
    ...
    const produtosObjetos = await Promise.all(
